...
test('getBrandProxy prefers a brand-specific override for PK storefronts', () => {
  const original = process.env.MALABIS_PROXY_KHAADI_PK;
  process.env.MALABIS_PROXY_KHAADI_PK = 'http://pk-proxy.local:8080';

  try {
    const brand = BRANDS.find((item) => item.key === 'khaadi-pk');
    assert.ok(brand);
    assert.equal(getBrandProxy(brand!), 'http://pk-proxy.local:8080');
  } finally {
    if (original === undefined) {
      delete process.env.MALABIS_PROXY_KHAADI_PK;
    } else {
      process.env.MALABIS_PROXY_KHAADI_PK = original;
    }
  }
});
